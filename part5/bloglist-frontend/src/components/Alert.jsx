...
const Alert = ({ event, setEvent }) => {
    return (
        ...
        <p className="inline mx-2 font-light">{event.message}</p>
        <button onClick={() => setEvent(null)} className="p-2 rounded-full text-green-600 dark:text-green-200 hover:bg-green-500 hover:bg-opacity-20 transition duration-150">
            <Undo className="inline " />
        </button>
        <button onClick={() => event.confirm()} className="p-2 rounded-full text-red-600 dark:text-red-200 hover:bg-red-500 hover:bg-opacity-30 transition duration-150">
            <Check className="inline text-red-400" strokeWidth={3} />
        </button>
    </div>
